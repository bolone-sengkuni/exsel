...
def show_main_menu(profile: dict, display_quota: str, segments: dict):
    ...
    expired_at_dt = (
        datetime.fromtimestamp(expired_at_ts).strftime("%Y-%m-%d %H:%M:%S")
        if expired_at_ts
        else "-"
    )
    ...
    info_table.add_row(" Name", f": 👤 [bold {theme['text_body']}]{profile['name']}[/]")
    info_table.add_row(
        " Nomor", f": 📞 [bold {theme['text_body']}]{profile['number']}[/]"
    )
    info_table.add_row(
        " Type",
        f": 🧾 [{theme['text_body']}]{profile['subscription_type']} ({profile['subscriber_id']})[/]",
    )
    info_table.add_row(" Pulsa", f": 💰 Rp [{theme['text_money']}]{pulsa_str}[/]")
    info_table.add_row(" Kuota", f": 📊 [{theme['text_date']}]{display_quota}[/]")
    info_table.add_row(
        " Tiering", f": 🏅 [{theme['text_date']}]{profile['point_info']}[/]"
    )
    ...
    #     console.print(
    #         Align.center(
    #             f"[{theme['text_sub']}]Pilih [Y] untuk lihat semua paket spesial[/{theme['text_sub']}]"
    #         )
    #     )
...
def show_main_menu2(active_user: dict, profile: dict):
    ...
    while True:
        ...
        choice = console.input(
            f"[{theme['text_sub']}]Pilih menu:[/{theme['text_sub']}] "
        ).strip()
        if choice == "10":
    ...
    while True:
        ...
        if active_user is not None:
            ...
            if total > 0 or has_unlimited:
                display_quota = f"{remaining / 1e9:.2f} / {total / 1e9:.2f} GB" + (
                    " (Unlimited)" if has_unlimited else ""
                )
            else:
                display_quota = "-"
            ...
            if choice.lower() == "t":
                ...
                print_success(
                    "🧹", f"Cache untuk akun {account_id} berhasil dibersihkan."
                )
                pause()
            elif choice == "69":
                show_theme_menu()
