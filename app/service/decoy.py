...
from app.menus.util import print_error, print_panel, print_success, print_warning
...
class DecoyPackage:
    ...
    def check_subscriber_change(self):
        ...
        if self.subscriber_id != current_subscriber_id:
            print_warning(
                "⚠️",
                f"Subscriber ID changed from {self.subscriber_id} to {current_subscriber_id}. Resetting decoy data.",
            )
            self.reset_decoys()
    ...
    def fetch_decoy_data(self, decoy_name: str):
        ...
        try:
            print_panel(
                "🔄 Refreshing Decoy", f"Refreshing decoy data for: {decoy_name}"
            )
            ...
            self.decoys[decoy_name] = {
                "option_code": decoy_package_detail["package_option"][
                    "package_option_code"
                ],
