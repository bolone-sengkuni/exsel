...
def make_ax_api_signature(
    ts_for_sign: str, contact: str, code: str, contact_type: str
) -> str:
...
def make_x_signature_bounty(
    ...
    msg = f"{access_token};{token_payment};{sig_time_sec};{package_code};".encode(
        "utf-8"
    )
...
def make_x_signature_bounty_allotment(
    ...
    msg = f"{token_confirmation};{sig_time_sec};{destination_msisdn};{package_code};".encode(
        "utf-8"
    )
