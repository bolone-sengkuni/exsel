...
from app.client.engsel import get_profile, update_profile
from app.menus.util import print_error, print_success, print_warning
...
class Auth:
    ...
    def add_refresh_token(self, number: int, refresh_token: str):
        existing = next(
            (rt for rt in self.refresh_tokens if rt["number"] == number), None
        )
        if existing:
            ...
            profile_data = get_profile(
                self.api_key, tokens["access_token"], tokens["id_token"]
            )
    ...
    def remove_refresh_token(self, number: int):
        self.refresh_tokens = [
            rt for rt in self.refresh_tokens if rt["number"] != number
        ]
        self.write_tokens_to_file()
        ...
        if self.active_user and self.active_user["number"] == number:
            if len(self.refresh_tokens) != 0:
                ...
                tokens = get_new_token(
                    self.api_key,
                    first_rt["refresh_token"],
                    first_rt.get("subscriber_id", ""),
                )
                if tokens:
    ...
    def set_active_user(self, number: int):
        ...
        tokens = get_new_token(
            self.api_key, rt_entry["refresh_token"], rt_entry.get("subscriber_id", "")
        )
        if not tokens:
            print_error(
                "❌",
                f"Failed to get tokens for number: {number}. Refresh token might be invalid or expired.",
            )
            return False
        ...
        rt_entry["name"] = profile_data["profile"]["name"]
    ...
    def get_active_user(self):
        ...
        if (
            ...
            or (int(time.time()) - self.last_refresh_time) > 300
        ):
    ...
    def edit_account_name(self, number: int, new_name: str) -> bool:
        nama = new_name.capitalize()
        for user in self.refresh_tokens:
            if user["number"] == number:
                user["name"] = nama
                break

        cek_acc = [i for i in self.refresh_tokens if i["number"] == number]
        if not cek_acc:
            print_error(
                "❌",
                f"Failed to get number {number}",
            )
            return False

        self.set_active_user(number)
        tokens = self.active_user["tokens"]
        access_token = tokens["access_token"]
        id_token = tokens["id_token"]

        profile = get_profile(
            api_key=self.api_key, access_token=access_token, id_token=id_token
        )
        if profile:
            db = profile["profile"]["dob"]
            dob = datetime.strptime(db, "%d/%m/%Y").strftime("%Y-%m-%d")
            if update_profile(
                ...
                self.write_tokens_to_file()
                print_success("✅", f"Account name for {number} updated to {nama}")
                return True
