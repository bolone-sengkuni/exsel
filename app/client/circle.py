...
from app.menus.util import live_loading, print_error, print_panel
...
def get_group_data(api_key: str, tokens: dict) -> dict:
    ...
    if not res or res.get("status") != "SUCCESS":
        print_error(
            "❌", f"Failed to fetch group detail: {res.get('error', 'Unknown error')}"
        )
...
def accept_circle_invitation(
    api_key: str, tokens: dict, group_id: str, member_id: str
) -> dict:
...
def create_circle(
    ...
    with live_loading(
        f"➕ Creating Circle with member {member_msisdn}...", get_theme()
    ):
        res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
    return res
...
def spending_tracker(
    api_key: str, tokens: dict, parent_subs_id: str, family_id: str
) -> dict:
