...
def send_api_request(
    api_key: str, path: str, payload_dict: dict, id_token: str, method: str = "POST"
):
            ...
            for ie in is_enterprise_list:
                ...
                if res.get("status") == "SUCCESS":
                    ...
                    if family_name:
                        ...
                        print_success(
                            "✅",
                            f"Success with is_enterprise={ie}, migration_type={mt}. Family name: {family_name}",
                        )
                        break
            if family_data:
                break
...
def get_families(api_key: str, tokens: dict, package_category_code: str) -> dict:
    ...
    with live_loading(
        f"📂 Fetching families for category {package_category_code}...", get_theme()
    ):
        res = send_api_request(api_key, path, payload_dict, tokens["id_token"], "POST")
    if res.get("status") != "SUCCESS":
        print_error(
            "❌", f"Gagal mengambil families untuk kategori {package_category_code}"
        )
...
def get_package(
    ...
    with live_loading("📦 Fetching package...", get_theme()):
...
def intercept_page(
    api_key: str, tokens: dict, option_code: str, is_enterprise: bool = False
):
...
def login_info(api_key: str, tokens: dict, is_enterprise: bool = False):
    ...
    if "data" not in res:
        print_error(
            "❌", f"Error getting login info: {res.get('error', 'Unknown error')}"
        )
        print_panel("📑 Response", json.dumps(res, indent=2))
        return None
    return res["data"]
...
def get_package_details(
    ...
    family_data = get_family(
        api_key, tokens, family_code, is_enterprise, migration_type
    )
    if not family_data:
...
def get_notifications(api_key: str, tokens: dict):
    ...
    if isinstance(res, dict) and res.get("status") != "SUCCESS":
        print_error(
            "❌", f"Error getting notifications: {res.get('error', 'Unknown error')}"
        )
        return None
    return res
...
def get_notification_detail(api_key: str, tokens: dict, notification_id: str):
    ...
    if isinstance(res, dict) and res.get("status") != "SUCCESS":
        print_error(
            "❌",
            f"Error getting notification detail: {res.get('error', 'Unknown error')}",
        )
        return None
    return res
    ...
    with live_loading(f"🚫 Unsubscribing quota {quota_code}...", get_theme()):
        try:
            res = send_api_request(
                api_key, path, raw_payload, tokens["id_token"], "POST"
            )
...
def dash_segments(
    api_key: str, id_token: str, access_token: str, balance: int = 0
) -> dict | None:
    path = "dashboard/api/v8/segments"
    payload = {
        "access_token": access_token,
...
def update_profile(
    api_key: str, access_token: str, id_token: str, name: str, dob: str
) -> bool:
    payload = {
        "lang": "en",
        "is_enterprise": False,
    ...
    try:
        resp = send_api_request(
            api_key,
            path="api/v8/update-profile",
            id_token=id_token,
            method="POST",
            payload_dict=payload,
        )
        if "code" in resp and resp.get("code") == "000":
            return True
        return False
    except Exception as e:
        print_error("❌", f"Gagal change name: {e}")
        return False

    # pp = get_profile(api_key=api_key, access_token=access_token, id_token=id_token)
    # print(pp)
