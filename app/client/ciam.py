...
from app.menus.util import live_loading, print_error, print_success
...
def submit_otp(api_key: str, contact_type: str, contact: str, code: str):
    ...
    signature = ax_api_signature(
        api_key, ts_for_sign, final_contact, code, contact_type
    )
...
def get_new_token(api_key: str, refresh_token: str, subscriber_id: str) -> dict | None:
    ...
    if "error" in body:
        print_error(
            "❌", f"Error: {body['error']} - {body.get('error_description', '')}"
        )
        return None
    return body
