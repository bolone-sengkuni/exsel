...
def build_encrypted_field(
    iv_hex16: str | None = None, urlsafe_b64: bool = False
) -> str:
...
def ax_api_signature(
    api_key: str, ts_for_sign: str, contact: str, code: str, contact_type: str
) -> str:
    return make_ax_api_signature(ts_for_sign, contact, code, contact_type)


def encryptsign_xdata(
    api_key: str, method: str, path: str, id_token: str, payload: dict
) -> dict:
...
def decrypt_xdata(api_key: str, encrypted_payload: dict) -> dict:
    if (
        ...
        or "xdata" not in encrypted_payload
        or "xtime" not in encrypted_payload
    ):
        raise ValueError(
            "Invalid encrypted data format. Expected dict with 'xdata' and 'xtime'."
        )
