...
def show_circle_creation(api_key: str, tokens: dict):
    ...
    parent_name = console.input(
        f"[{theme['text_sub']}]Enter your name (Parent):[/{theme['text_sub']}] "
    ).strip()
    group_name = console.input(
        f"[{theme['text_sub']}]Enter Circle name:[/{theme['text_sub']}] "
    ).strip()
    member_msisdn = console.input(
        f"[{theme['text_sub']}]Enter initial member's MSISDN (e.g., 6281234567890):[/{theme['text_sub']}] "
    ).strip()
    member_name = console.input(
        f"[{theme['text_sub']}]Enter initial member's name:[/{theme['text_sub']}] "
    ).strip()

    with live_loading("🔄 Creating circle...", theme):
        create_res = create_circle(
            api_key, tokens, parent_name, group_name, member_msisdn, member_name
        )
...
def show_bonus_list(api_key: str, tokens: dict, parent_subs_id: str, family_id: str):
    ...
    while in_circle_bonus_menu:
        ...
        for idx, bonus in enumerate(bonus_list, start=1):
            ...
            console.print(
                Panel(bonus_text, border_style=theme["border_primary"], expand=True)
            )
        ...
        choice = console.input(
            f"[{theme['text_sub']}]Pilih opsi:[/{theme['text_sub']}] "
        ).strip()
        if choice == "00":
            ...
            if action_type == "PLP":
                ...
                console.print(
                    Panel(detail_text, border_style=theme["border_info"], expand=True)
                )
                pause()
    ...
    while in_circle_menu:
        ...
        if group_id == "":
            console.print(
                Panel(
                    Align.center(
                        "🚫 You are not part of any Circle.", vertical="middle"
                    ),
                    border_style=theme["border_info"],
                    padding=(1, 2),
                    expand=True,
            ...
            create_new = (
                console.input(
                    f"[{theme['text_sub']}]Create a new Circle? (y/n):[/{theme['text_sub']}] "
                )
                .strip()
                .lower()
            )
        ...
        for member in members:
            if member.get("member_role") == "PARENT":
                ...
                parrent_msisdn = decrypt_circle_msisdn(
                    api_key, member.get("msisdn", "")
                )
        ...
        header_text.append(
            f"Owner: {owner_name} {parrent_msisdn}\n", style=theme["text_body"]
        )
        header_text.append(
            f"Package: {package_name} | {formatted_remaining} / {formatted_allocation}\n",
            style=theme["text_body"],
        )
        header_text.append(
            f"Spending: {get_rupiah(spend)} / {get_rupiah(target)}\n",
            style=theme["text_money"],
        )

        console.print(
            Panel(header_text, border_style=theme["border_primary"], expand=True)
        )

        for idx, m in enumerate(members, start=1):
            msisdn = (
                decrypt_circle_msisdn(api_key, m.get("msisdn", "")) or "<No Number>"
            )
            me_mark = " (You)" if str(msisdn) == str(my_msisdn) else ""
            role = "Parent" if m.get("member_role") == "PARENT" else "Member"
            joined_str = (
                datetime.fromtimestamp(m.get("join_date", 0)).strftime("%Y-%m-%d")
                if m.get("join_date")
                else "N/A"
            )
            alloc = format_quota_byte(m.get("allocation", 0))
            used = format_quota_byte(
                max(m.get("allocation", 0) - m.get("remaining", 0), 0)
            )
            usage_str = f"{used} / {alloc}"

            member_text = Text()
            member_text.append(
                f"{idx}. {m.get('member_name', 'N/A')} {me_mark}\n", style="bold"
            )
            ...
            console.print(
                Panel(member_text, border_style=theme["border_info"], expand=True)
            )
        ...
        console.print(
            Panel(nav_table, border_style=theme["border_primary"], expand=True)
        )

        choice = console.input(
            f"[{theme['text_sub']}]Pilihan:[/{theme['text_sub']}] "
        ).strip()
        if choice == "00":
            return "BACK"

        elif choice == "1":
            msisdn_to_invite = console.input(
                f"[{theme['text_sub']}]Enter MSISDN to invite:[/{theme['text_sub']}] "
            ).strip()
            ...
            if validate_res.get("status") == "SUCCESS":
                if validate_res.get("data", {}).get("response_code", "") != "200-2001":
                    print_panel(
                        "⚠️ Info",
                        f"Cannot invite {msisdn_to_invite}: {validate_res.get('data', {}).get('message', 'Unknown error')}",
                    )
                    pause()
                    continue
            member_name = console.input(
                f"[{theme['text_sub']}]Enter member name:[/{theme['text_sub']}] "
            ).strip()
            ...
            if (
                invite_res.get("status") == "SUCCESS"
                and invite_res.get("data", {}).get("response_code") == "200-00"
            ):
            ...
            try:
                ...
                if is_last_member:
                    print_panel(
                        "⚠️ Info", "Cannot remove the last member from the Circle."
                    )
                    pause()
                    continue
                msisdn_to_remove = decrypt_circle_msisdn(
                    api_key, member_to_remove.get("msisdn", "")
                )
                confirm = (
                    console.input(
                        f"[{theme['text_sub']}]Remove {msisdn_to_remove}? (y/n):[/{theme['text_sub']}] "
                    )
                    .strip()
                    .lower()
                )
                ...
                if remove_res.get("status") == "SUCCESS":
                    ...
                    res_text.append(
                        json.dumps(remove_res, indent=2), style=theme["text_body"]
                    )
                    console.print(
                        Panel(res_text, border_style=theme["border_info"], expand=True)
                    )
                else:
            ...
            try:
                ...
                if member_to_accept.get("status") != "INVITED":
                    ...
                    pause()
                    continue
                msisdn_to_accept = decrypt_circle_msisdn(
                    api_key, member_to_accept.get("msisdn", "")
                )
                confirm = (
                    console.input(
                ...
                with live_loading("🔄 Accepting invitation...", theme):
                    accept_res = accept_circle_invitation(
                        api_key, tokens, group_id, member_to_accept.get("member_id", "")
                    )
                if accept_res.get("status") == "SUCCESS":
                    res_text = Text()
                    res_text.append(
                        f"Invitation for {msisdn_to_accept} accepted.\n", style="bold"
                    )
                    res_text.append(
                        json.dumps(accept_res, indent=2), style=theme["text_body"]
                    )
                    console.print(
                        Panel(res_text, border_style=theme["border_info"], expand=True)
                    )
