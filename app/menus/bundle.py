...
from app.menus.util import clear_screen, get_rupiah, pause, print_panel, simple_number
...
def show_bundle_menu():
    ...
    while True:
        ...
        if cart_items:
            ...
            console.print(
                f"[{theme['text_body']}]Total Harga: Rp {get_rupiah(total_price)}[/]"
            )
        else:
        ...
        console.print(
            Panel(
                nav, border_style=theme["border_primary"], padding=(0, 0), expand=True
            )
        )
        ...
        if choice == "1":
            ...
            fc = console.input(
                f"[{theme['text_sub']}]Masukkan Family Code:[/{theme['text_sub']}] "
            ).strip()
            ...
            idx = console.input(
                f"[{theme['text_sub']}]Nomor item yang ingin dihapus:[/{theme['text_sub']}] "
            ).strip()
            ...
            for i, item in enumerate(display_cart, start=1):
                info_text.append(
                    f"{i}. {item['name']} - Rp {get_rupiah(item['price'])}\n",
                    style=theme["text_body"],
                )
            info_text.append(
            ...
            while True:
                ...
                confirm = (
                    console.input(
                        f"[{theme['text_sub']}]Lanjutkan pembelian dengan metode ini? (y/n):[/{theme['text_sub']}] "
                    )
                    .strip()
                    .lower()
                )
                ...
                if method == "1":
                    ...
                    console.input(
                        f"[{theme['text_sub']}]✅ Pembayaran selesai. Tekan Enter...[/{theme['text_sub']}]"
                    )
                    break
                elif method == "2":
