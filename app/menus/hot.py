...
# Import only what we need directly to avoid circular imports via
# `app.config.imports` (which re-exports menu functions and imports
# this module, causing PaymentItem to sometimes be undefined).
...
def show_hot_menu():
    ...
    while in_hot_menu:
        ...
        for idx, p in enumerate(hot_packages, start=1):
            table.add_row(
                str(idx), p["family_name"], p["variant_name"], p["option_name"]
            )
        ...
        choice = console.input(
            f"[{theme['text_sub']}]Pilih paket (nomor):[/{theme['text_sub']}] "
        ).strip()
        if choice == "00":
            in_hot_menu = False
            return None
...
def show_hot_menu2():
    ...
    while in_bookmark_menu:
        ...
        pkg_table.add_column(
            "Harga", justify="right", style=theme["text_money"], width=16
        )
        ...
        choice = console.input(
            f"[{theme['text_sub']}]Pilih paket:[/{theme['text_sub']}] "
        ).strip()
        if choice == "00":
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(hot_packages):
            ...
            info_text.append(
                f"Harga: Rp {get_rupiah(selected_package['price'])}\n",
            ...
            while in_payment_menu:
                method_table = Table(
                    show_header=False, box=MINIMAL_DOUBLE_HEAD, expand=True
                )
                method_table.add_column(
                    justify="right", style=theme["text_key"], width=6
                )
                ...
                method_table.add_row(
                    "00", f"[{theme['text_sub']}]Kembali ke menu sebelumnya[/]"
                )
                ...
                input_method = console.input(
                    f"[{theme['text_sub']}]Pilih metode:[/{theme['text_sub']}] "
                ).strip()

                if input_method == "1":
                    if overwrite_amount == -1:
                        last_price = (
                            payment_items[-1].item_price
                            if hasattr(payment_items[-1], "item_price")
                            else payment_items[-1]["item_price"]
                        )
                        warn_price_str = (
                            get_rupiah(last_price)
                            if isinstance(last_price, (int, float))
                            else str(last_price)
                        )
                        console.print(
                            f"[{theme['text_warn']}]Pastikan sisa balance KURANG DARI Rp{warn_price_str}!!![/]"
                        )

                        balance_answer = console.input(
                            f"[{theme['text_sub']}]Apakah anda yakin ingin melanjutkan pembelian? (y/n):[/{theme['text_sub']}] "
                        ).strip()
                    ...
                    console.input(
                        f"[{theme['text_sub']}]Tekan enter untuk kembali...[/{theme['text_sub']}] "
                    )
                    in_payment_menu = False
                    in_bookmark_menu = False
