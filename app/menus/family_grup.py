...
def ensure_family_file():
    ...
    try:
        ...
        if (
            ...
            or "codes" not in data
            or not isinstance(data["codes"], list)
        ):
    ...
    while True:
        ...
        if semua_kode:
            ...
            for i, item in enumerate(semua_kode, start=1):
                ...
                packages.append(
                    {"number": i, "code": item["code"], "name": item["name"]}
                )
        ...
        aksi = (
            console.input(
                f"[{theme['text_title']}]Pilih aksi atau nomor kode:[/{theme['text_title']}] "
            )
            .strip()
            .lower()
        )
        ...
        if aksi == "t":
            ...
            print_panel(
                ...
                if success
                else "Gagal menambahkan, family code sudah ada di daftar.",
            )
            pause()
            ...
            if idx.isdigit() and 1 <= int(idx) <= len(semua_kode):
                ...
                konfirmasi = (
                    console.input(f"Yakin ingin menghapus '{nama}' ({kode})? (y/n): ")
                    .strip()
                    .lower()
                )
                if konfirmasi == "y":
            ...
            if not semua_kode:
                ...
                continue
            idx = console.input(
                "Masukkan nomor kode yang ingin diubah namanya: "
            ).strip()
            if idx.isdigit() and 1 <= int(idx) <= len(semua_kode):
            ...
            if selected:
                try:
                    result = get_packages_by_family(
                        selected["code"], return_package_detail=return_package_detail
                    )
