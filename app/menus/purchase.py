...
# Import only the names used here to avoid circular imports caused by
# `from app.config.imports import *` which re-exports modules that
# may import this module back during package initialization.
...
def purchase_loop(
    ...
    if not family_data:
        print_panel(
            "❌ Error", f"Gagal mengambil data family untuk kode: {family_code}."
        )
        pause()
        return False
    ...
    if not target_option or not target_variant:
        print_panel(
            "❌ Error", f"Option order {order} tidak ditemukan di family {family_code}."
        )
        pause()
        return False
    ...
    console.print("-" * 55)
    console.print(
        f"[{theme['text_title']}]Mencoba beli: {target_variant['name']} - {order}. {option_name} - Rp{option_price}[/]"
    )
...
def purchase_by_family(
    ...
    if use_decoy:
        ...
        console.print(
            f"[{theme['text_warn']}]Pastikan sisa balance KURANG DARI Rp{balance_threshold}![/]"
        )
        balance_answer = (
            console.input("Apakah anda yakin ingin melanjutkan pembelian? (y/n): ")
            .strip()
            .lower()
        )
        if balance_answer != "y":
    ...
    for variant in variants:
        ...
        for option in variant["package_options"]:
            ...
            if not start_buying:
                console.print(
                    f"[{theme['text_sub']}]Skipping option {option_order}. {option['name']}[/]"
                )
                continue
            ...
            purchase_count += 1
            console.print(
                f"[{theme['text_title']}]Purchase {purchase_count} dari {packages_count}...[/]"
            )
            console.print(
                f"Mencoba beli: {variant_name} - {option_order}. {option_name} - Rp{option_price}"
            )
            ...
            try:
                if use_decoy:
                    ...
                    decoy_package_detail = get_package(
                        api_key, tokens, decoy["option_code"]
                    )
                    if not decoy_package_detail:
                        print_panel("❌ Error", "Gagal memuat detail paket decoy.")
                        pause()
                ...
                print_panel("❌ Error", f"Exception saat mengambil detail paket: {e}")
                console.print(
                    f"Gagal ambil detail untuk {variant_name} - {option_name}. Melewati."
                )
                continue

            payment_items.append(
                PaymentItem(
                    item_code=target_package_detail["package_option"][
                        "package_option_code"
                    ],
                    product_type="",
                    item_price=target_package_detail["package_option"]["price"],
            ...
            if use_decoy:
                payment_items.append(
                    PaymentItem(
                        item_code=decoy_package_detail["package_option"][
                            "package_option_code"
                        ],
                        product_type="",
                        item_price=decoy_package_detail["package_option"]["price"],
            ...
            try:
                ...
                if res and res.get("status", "") != "SUCCESS":
                    ...
                    if "Bizz-err.Amount.Total" in error_msg:
                        ...
                        valid_amount = int(error_msg_arr[1].strip())
                        print_panel(
                            "ℹ️ Info", f"Adjusted total amount to: {valid_amount}"
                        )
                        res = settlement_balance(
                            api_key,
                            tokens,
            ...
            if delay_seconds > 0 and should_delay:
                console.print(
                    f"[{theme['text_sub']}]Menunggu {delay_seconds} detik sebelum pembelian berikutnya...[/]"
                )
...
def purchase_n_times(
    ...
    variants = family_data["package_variants"]
    target_variant = next(
        (v for v in variants if v["package_variant_code"] == variant_code), None
    )
    if not target_variant:
        print_panel(
            "❌ Error",
            f"Variant code {variant_code} tidak ditemukan di family {family_name}.",
        )
        pause()
        return None

    target_option = next(
        (o for o in target_variant["package_options"] if o["order"] == option_order),
        None,
    )
    if not target_option:
        print_panel(
            "❌ Error",
            f"Option order {option_order} tidak ditemukan di variant {target_variant['name']}.",
        )
        pause()
        return None
    ...
    for i in range(n):
        ...
        console.print(
            f"Mencoba beli: {target_variant['name']} - {option_order}. {option_name} - Rp{option_price}"
        )
        ...
        try:
            ...
            print_panel("❌ Error", f"Exception saat mengambil detail paket: {e}")
            console.print(
                f"Gagal ambil detail untuk {target_variant['name']} - {option_name}. Melewati."
            )
            continue
    ...
    console.print(
        f"[{theme['text_title']}]Total pembelian sukses {len(successful_purchases)}/{n}[/]"
    )
    console.print(
