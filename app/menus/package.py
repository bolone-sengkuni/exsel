...
def show_package_details(
    api_key, tokens, package_option_code, is_enterprise, option_order=-1
):
    theme = get_theme()
    clear_screen()
    ...
    if benefits:
        ...
        for b in benefits:
            ...
            if is_unli:
                total_str = {"VOICE": "menit", "TEXT": "SMS", "DATA": "kuota"}.get(
                    dt, dt
                )
            else:
                if dt == "VOICE":
    ...
    if choice == "00":
        ...
        console.print(
            f"Harga Paket Decoy: Rp {decoy_package_detail['package_option']['price']}"
        )
        ...
        use_decoy_for_n_times = (
            console.input("Use decoy package? (y/n): ").strip().lower() == "y"
        )
        n_times_str = console.input(
            "Enter number of times to purchase (e.g., 3): "
        ).strip()
        delay_seconds_str = console.input(
            "Enter delay between purchases in seconds (e.g., 25): "
        ).strip()
        ...
        try:
            ...
            print_panel(
                "❌ Error", "Invalid number entered. Masukkan integer yang valid."
            )
            pause()
            return "BACK"
        ...
        destination_msisdn = console.input(
            "Masukkan nomor tujuan bonus (mulai dengan 62): "
        ).strip()
        bounty_allotment(
            api_key=api_key,
            tokens=tokens,
...
def get_packages_by_family(
    ...
    while True:
        ...
        info_text.append(
            f"{data['package_family']['package_family_type']}\n",
            style=theme["text_value"],
        )
        info_text.append("Jumlah Varian: ", style=theme["border_info"])
        info_text.append(
            f"{len(data['package_variants'])}\n", style=theme["text_value"]
        )
        ...
        for pkg in packages:
            harga_str = (
                ...
                if price_currency == "Rp"
                else f"{pkg['price']} Poin"
            )
            table.add_row(
                str(pkg["number"]), pkg["variant_name"], pkg["option_name"], harga_str
            )

        console.print(
            Panel(table, border_style=theme["border_info"], padding=(0, 0), expand=True)
        )
        ...
        console.print(
            Panel(
                nav, border_style=theme["border_primary"], padding=(0, 1), expand=True
            )
        )

        choice = console.input(
            f"[{theme['text_title']}]Pilih paket (nomor):[/{theme['text_title']}] "
        ).strip()
        if choice == "00":
            return "BACK"
        ...
        if return_package_detail:
            variant_code = next(
                (
                    v["package_variant_code"]
                    for v in data["package_variants"]
                    if v["name"] == selected["variant_name"]
                ),
                None,
            )
    ...
    while True:
        ...
        for num, quota in enumerate(quotas, start=1):
            ...
            if benefits:
                ...
                benefit_table.add_column("Nama", style=theme["text_body"])
                benefit_table.add_column("Jenis", style=theme["text_body"])
                benefit_table.add_column(
                    "Kuota", style=theme["text_body"], justify="right"
                )
        ...
        nav_table.add_row(
            nav_range("del", len(my_packages)), f"[{theme['text_err']}]Hapus paket[/]"
        )
        ...
        if choice.isdigit():
            ...
            confirm = (
                console.input(
                    f"[{theme['text_sub']}]Yakin ingin unsubscribe dari paket {nomor}. {selected['name']}? (y/n):[/{theme['text_sub']}] "
                )
                .strip()
                .lower()
            )
            if confirm == "y":
                ...
                print_panel(
                    ...
                    "Berhasil unsubscribe." if success else "Gagal unsubscribe.",
                )
            else:
