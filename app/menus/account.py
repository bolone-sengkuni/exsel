...
from app.config.imports import MINIMAL_DOUBLE_HEAD, Align, Panel, Table, get_theme
...
def login_prompt(api_key: str):
    ...
    console.print("Masukkan nomor XL, Supported (08xx / 628xx / +628xx) ")
    raw_input = console.input(
        f"[{theme['text_sub']}] Nomor: [/{theme['text_sub']}] "
    ).strip()
    phone_number = normalize_number(raw_input)

    if (
        not phone_number.startswith("628")
        or len(phone_number) < 10
        or len(phone_number) > 14
    ):
    ...
    try:
        ...
        for attempt in range(1, 6):
            otp = console.input(
                f"Percobaan {attempt}/5 - Masukkan OTP (6 digit): "
            ).strip()
...
def show_account_menu():
    ...
    while in_account_menu:
        ...
        account_table.add_column(
            "No", style=theme["text_key"], justify="right", width=3
        )
        ...
        if input_str == "00":
            ...
            nomor_input = console.input(
                f"Nomor akun yang ingin diedit (1 - {len(users)}): "
            ).strip()
            if nomor_input.isdigit():
                ...
                if 1 <= nomor <= len(users):
                    ...
                    new_name = console.input(
                        f"Masukkan nama baru untuk akun {selected_user['number']}: "
                    ).strip()
                    if new_name:
                        if AuthInstance.edit_account_name(
                            selected_user["number"], new_name
                        ):
                            AuthInstance.load_tokens()
                            users = AuthInstance.refresh_tokens
                            active_user = AuthInstance.get_active_user()
            ...
            nomor_input = console.input(
                f"Nomor akun yang ingin dihapus (1 - {len(users)}): "
            ).strip()
            if nomor_input.isdigit():
                nomor = int(nomor_input)
                if 1 <= nomor <= len(users):
                    ...
                    confirm = (
                        console.input(
                            f"Yakin ingin menghapus akun {selected_user['number']}? (y/n): "
                        )
                        .strip()
                        .lower()
                    )
                    if confirm == "y":
                        print_panel(
                            "⏳ Info", f"Menghapus akun {selected_user['number']}..."
                        )
                        ...
                        print_panel(
                            "✅ Info",
                            f"Akun {selected_user['number']} berhasil dihapus.",
                        )
                    else:
