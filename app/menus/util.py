...
def print_banner():
    theme = get_theme()
    banner_text = Align.center("[bold]MyXl v8.9.1 Dor[/]", vertical="middle")
    console.print(
        Panel(
            banner_text,
...
def simple_number():
    ...
    if not active_user:
        ...
        text = f"[bold {get_theme_style('text_body')}]Akun ({active_user.get('name')}) aktif ✨ {number} ✨[/]"
...
|___|\__/|___||___/     |___/\___| \_______) """
    ...
    content = f"{ascii_art}\n                  {version_text}"
    console.print(
        Padding(Align.center(content), (1, 0)), style=get_theme_style("text_sub")
    )
    # console.print(
    #    Panel(
...
def pause():
    console.print(
        Panel(
            "⏸️ Press Enter to continue...", border_style=get_theme_style("border_info")
        )
    )
    input()
...
class HTMLToText(HTMLParser):
    ...
    def get_text(self):
        ...
        text = re.sub(r"\n\s*\n\s*\n+", "\n\n", text)
        return "\n".join(
            textwrap.wrap(text, width=self.width, replace_whitespace=False)
        )
...
def live_loading(text: str, theme: dict):
    return console.status(
        f"[{theme['text_body']}]{text}[/{theme['text_body']}]", spinner="dots"
    )
...
def print_title(text):
    console.print(
        Panel(
            Align.center(
                f"[bold {get_theme_style('text_title')}]{text}[/{get_theme_style('text_title')}]"
            ),
            border_style=get_theme_style("border_primary"),
            padding=(0, 1),
            expand=True,
...
def print_key_value(label, value):
    console.print(
        f"[{get_theme_style('text_key')}]{label}:[/] [{get_theme_style('text_value')}]{value}[/{get_theme_style('text_value')}] ✅"
    )


def print_info(label, value):
    console.print(
        f"[{get_theme_style('text_sub')}]{label}:[/{get_theme_style('text_sub')}] [{get_theme_style('text_body')}]{value}[/{get_theme_style('text_body')}]"
    )
