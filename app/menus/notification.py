...
from app.menus.util import clear_screen, live_loading, pause, print_panel, simple_number
...
def show_notification_menu():
    ...
    while in_notification_menu:
        ...
        notifications = (
            notifications_res.get("data", {}).get("notification", {}).get("data", [])
        )
        ...
        if choice == "1":
            for notification in notifications:
                ...
                if detail:
                    print_panel(
                        "✅ Info", f"Mark as READ notification ID: {notification_id}"
                    )
            pause()

        elif choice == "2":
            nomor = console.input(
                f"[{theme['text_sub']}]Masukkan nomor notifikasi:[/{theme['text_sub']}] "
            ).strip()
            ...
            nomor = int(nomor)
            selected = next(
                (n for i, n in enumerate(notifications, start=1) if i == nomor), None
            )
            if not selected:
